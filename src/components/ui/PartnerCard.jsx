...
const PartnerCard = ({ imageurl, title, content, btntxt, index }) => {
  ...
  return (
    <section className="grid grid-cols-1 md:grid-cols-2 py-4 md:py-8 gap-2">
      <section className={`flex items-center ${index % 2 === 0 ? 'order-2 md:order-1' : 'order-1 md:order-2'}`}>
        <img src={imageurl} className="rounded-lg object-cover w-full md:h-[70vh] md:w-10/12"/>
      </section>
      <section className={`flex flex-col justify-center ${index % 2 === 0 ? 'order-1 md:order-2' : 'order-2 md:order-1'}`}>
